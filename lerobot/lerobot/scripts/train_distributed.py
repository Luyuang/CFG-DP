...
def train(cfg: TrainPipelineConfig, accelerator: Callable = None):
    ...
    logging.warning(colored("Dataloader length: ", "yellow", attrs=["bold"]) + f"{len(dataloader)}")
    logging.warning(colored("Dataloader batch size: ", "yellow", attrs=["bold"]) + f"{cfg.batch_size}")
    ...
    for _ in range(step, cfg.steps):
        ...
        #     'grad_norm': f"{train_tracker.grad_norm.avg:.1f}"
        # })
