...
class PushTImageDataset(BaseImageDataset):
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        data = {
            'agent_pos': np.hstack((self.replay_buffer['state_joint_with_hand'][:,:8],self.replay_buffer['state_joint_with_hand'][:,13:21])),
            'action': np.hstack((self.replay_buffer['cmd_joint_with_hand'][:,:8],self.replay_buffer['cmd_joint_with_hand'][:,13:21])),
        }
    ...
    def _sample_to_data(self, sample):
        ...
        data['obs']['agent_pos'] = np.hstack((agent_pos[:, 0:8], agent_pos[:, 13:21]))
        data['action'] = np.hstack((action[:, :8], action[:,13:21]))
